...
class VisualizationApp {
    constructor() {
        ...
        // State
        this.state = {
            mode: 'story', // 'story', 'explore', or 'similarity'
            filterShape: 'all',
            mapMode: 'count',
            activeCity: null,
            activeItem: null,
    ...
    async init() {
        try {
            ...
            window.addEventListener('resize', () => {
                ...
                if (this.state.mode === 'similarity') this.renderSimilarityFriends();
            });

            // Lazy load Part 3 data in background
            this.initSimilarityData();
    ...
    // --- Part 3: Similarity Logic ---

    async initSimilarityData() {
        try {
            const [dataRes, listRes] = await Promise.all([
                fetch('part3_data/pottery_data.json'),
                fetch('part3_data/pottery_list.json')
            ]);
            this.state.similarityData = await dataRes.json();
            this.state.similarityList = await listRes.json();
            document.getElementById('sim-loading').style.display = 'none';
        } catch (e) {
            console.error("Failed to load similarity data:", e);
        }
    }

    enterSimilarityMode() {
        this.state.mode = 'similarity';
        const simUI = document.getElementById('similarity-interface');
        simUI.style.display = 'flex';
        simUI.style.pointerEvents = 'auto';
        setTimeout(() => simUI.style.opacity = '1', 10);

        if (!this.state.currentSimId && this.state.similarityList.length > 0) {
            this.loadRandomSimilarity();
        }
    }
    ...
    exitSimilarityMode() {
        ...
        simUI.style.pointerEvents = 'none';
        setTimeout(() => simUI.style.display = 'none', 1000);
    }

    loadRandomSimilarity() {
        if (!this.state.similarityList.length) return;
        const random = this.state.similarityList[Math.floor(Math.random() * this.state.similarityList.length)];
        this.loadSimilarityPottery(random.id);
    }

    loadSimilarityPottery(id) {
        const pottery = this.state.similarityData[id];
        if (!pottery) return;

        this.state.currentSimId = id;
        
        // Update Center
        document.getElementById('sim-center-img').src = `part3_data/images/${id}.jpg`;
        document.getElementById('sim-center-name').innerText = pottery.name;

        // Render Friends
        this.renderSimilarityFriends();
    }

    renderSimilarityFriends() {
        const pottery = this.state.similarityData[this.state.currentSimId];
        const container = document.getElementById('sim-friends-ring');
        const vizBox = document.getElementById('sim-viz-container');
        if (!pottery || !container || !vizBox) return;

        container.innerHTML = '';
        const friends = pottery.friends.slice(0, 10);
        ...
        friends.forEach((friend, i) => {
            ...
            item.className = 'absolute transform -translate-x-1/2 -translate-y-1/2 pointer-events-auto cursor-pointer group';
            item.style.left = `${x}px`;
            item.style.top = `${y}px`;
            
            const sim = (friend.similarity * 100).toFixed(1);
            
            item.innerHTML = `
                <div class="flex flex-col items-center">
                    <div class="w-24 h-24 rounded-full border-4 border-white shadow-lg overflow-hidden transition-transform duration-300 group-hover:scale-110 group-hover:border-[#8d6e63]">
                        <img src="part3_data/images/${friend.id}.jpg" class="w-full h-full object-cover">
                    </div>
                    <div class="mt-2 bg-white/90 backdrop-blur px-2 py-0.5 rounded shadow-sm text-[10px] font-bold text-[#4CAF50]">相似度 ${sim}%</div>
                    <div class="text-[10px] font-bold text-[#3e2723] bg-white/80 px-2 py-0.5 rounded mt-1 opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap max-w-[120px] truncate">${friend.name}</div>
                </div>
            `;
            
            item.onclick = () => this.loadSimilarityPottery(friend.id);
            container.appendChild(item);
        });
    }

    searchSimilarity() {
        const query = document.getElementById('sim-search-input').value.trim().toLowerCase();
        if (!query) return;
        
        const result = this.state.similarityList.find(p => p.name.toLowerCase().includes(query));
        if (result) {
            this.loadSimilarityPottery(result.id);
        } else {
            alert("未找到相关陶器");
        }
    }
